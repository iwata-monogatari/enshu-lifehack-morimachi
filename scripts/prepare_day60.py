...
S1="https://www.town.morimachi.shizuoka.jp/material/files/group/10/cyekkurisuto.pdf"
...
S4="https://www.town.morimachi.shizuoka.jp/gyosei/machinososhiki/juminseikatsuka/seikatsukankyokakari/2/3385.html"
S5="https://www.town.morimachi.shizuoka.jp/gyosei/machinososhiki/norinseisakuka/rinseikakari/3/index.html"
S6="https://www.pref.shizuoka.jp/kurashikankyo/shizenkankyo/wild/1017692.html"
sections=[
("動物名を決める前に痕跡一件票を開く",[
...
facts=[
("森町の空き家バンク確認票は目視による害獣被害の有無を独立項目にする",S1),
("森町の確認票は雨漏り・白蟻・外壁・躯体基礎・害獣・残置物を分ける",S1),
("森町の第2期空家等対策計画は令和4年度の実態調査を踏まえて策定された",S2),
("森町の第2期空家等対策計画は森町全域を対象とする",S2),
("森町の計画資料は動物の棲みつきと鳴き声による生活環境影響を挙げる",S3),
("森町の計画資料は糞尿等による悪臭を生活環境影響として挙げる",S3),
("森町の計画資料は毛や羽の大量飛散を生活環境影響として挙げる",S3),
("森町の計画資料はねずみ・はえ・蚊等の大量発生を生活環境影響として挙げる",S3),
("森町の計画資料は周辺の土地・家屋への動物侵入を生活環境影響として挙げる",S3),
("森町の蜂案内は空き家等で所有者・管理者不明の場合に住民生活課への連絡を案内する",S4),
("森町の蜂案内は人に害のない場所の巣では寒くなるまでそっとする選択肢も示す",S4),
("森町の有害鳥獣ページは鳥獣被害防止計画と野生動物目撃情報への入口を掲載する",S5),
("静岡県は傷病野生鳥獣を安易に持ち帰らないよう案内する",S6),
("静岡県は傷病野生鳥獣を動物園へ直接問い合わせ・持込みしないよう案内する",S6),
("静岡県はすべての傷病野生鳥獣を回収・治療するわけではないと説明する",S6),
("静岡県は保護対象を希少種や渡り鳥等の条件に照らして扱うと説明する",S6)]
rec={"id":128,"verified_facts":[{"statement":s,"source_url":u} for s,u in facts],"morimachi_conditions":["動物名を自己判断せず痕跡と確認者を分ける","生体確認前に侵入口を塞がない","衛生清掃と建物修繕を別工程にする","捕獲・回収・保護対象は町県や専門家へ確認する"],"section_headings":[h for h,_ in sections],"section_paragraphs":[{"heading":h,"paragraphs":p} for h,p in sections],"faqs":[
{"question":"空き家で動物の痕跡を見つけたら最初に何をしますか？","answer":"触れずに退避できる状態を確保し、生体、死骸、糞尿、毛・足跡、巣、音・臭いを分け、場所・時刻・写真番号を残して町や専門業者へ相談します。"},
{"question":"侵入口らしい穴はすぐ塞いでよいですか？","answer":"内部に生体や幼獣が残る可能性があるため、先に塞ぎません。専門業者へ生体確認、退出方法、全侵入口の調査、封鎖と修繕の順を確認します。"},
{"question":"糞や死骸を自分で回収して種類を調べてもよいですか？","answer":"素手で触れたり持ち帰ったりしません。位置、範囲、状態を安全な距離から記録し、回収・清掃・消毒方法を所管や専門業者へ確認してください。"},
{"question":"清掃が終われば害獣対応は完了ですか？","answer":"清掃だけでは完了しません。生体不在の確認、侵入口封鎖、汚染材や建物の修繕、再発確認を別工程として記録します。"}],"sources":[
{"url":S1,"title":"森町 空き家・空き地バンクチェックリスト","status":"verified-http-200","checked_at":"2026-08-16"},
...
{"url":S4,"title":"森町 蜂の駆除について","status":"verified-http-200","checked_at":"2026-08-16"},
{"url":S5,"title":"森町 有害鳥獣","status":"verified-http-200","checked_at":"2026-08-16"},
{"url":S6,"title":"静岡県 けがをした野生鳥獣を見つけたら","status":"verified-http-200","checked_at":"2026-08-16"}]}
(ROOT/"work"/"day60-research-128.json").write_text(json.dumps({"records":[rec]},ensure_ascii=False,indent=2)+"\n",encoding="utf8")
topics=json.loads((ROOT/"data"/"seo-phase4-topics.json").read_text(encoding="utf8"))
for t in topics:
 if t["id"]==128:
  t["title"]="森町の空き家で動物の痕跡を見つけたら｜接触せず工程を分ける記録票"
  t["search_intent"]="空き家の糞・毛・足跡・音を安全に記録し、生体確認・衛生清掃・侵入口封鎖・建物修繕へ分けたい"
...
def update_public_surfaces():
 u="/vacant-house/vacant-house-animal-entry/";full="https://morimachi.enshu-lifehack.com"+u
 p=ROOT/"sitemap.xml";s=p.read_text(encoding="utf8")
 if full not in s:p.write_text(s.replace("</urlset>",f'<url><loc>{full}</loc><lastmod>2026-08-16</lastmod></url>\n</urlset>'),encoding="utf8")
 p=ROOT/"search-index.json";a=json.loads(p.read_text(encoding="utf8"))
 if not any(x.get("href")==u for x in a):
  a.append({"href":u,"icon":"🏠","title":"森町の空き家で動物の痕跡を見つけたら｜接触せず工程を分ける記録票","category":"空き家","hub":"property","kind":"点検","summary":"動物の痕跡を触れず記録し、生体・衛生・封鎖・修繕へ分ける","keyword":"森町 空き家 動物 糞 足跡 侵入口","aliases":["空き家害獣痕跡","動物侵入記録","空き家糞清掃"],"needs":["空き家の動物痕跡を安全に処理したい"],"department":["住民生活課","農林担当","定住推進課"],"audience":["空き家所有者","遠方の家族"],"verified":"2026-08-16","lead":"動物名を急いで決めず、痕跡を安全に記録して生体確認・衛生・封鎖・修繕へ渡す記録票です。"})
  p.write_text(json.dumps(a,ensure_ascii=False,separators=(",",":")),encoding="utf8")
 p=ROOT/"index.html";p.write_text(p.read_text(encoding="utf8").replace("サイト全体の公開ページは 784 件です","サイト全体の公開ページは 785 件です"),encoding="utf8")
