...
def scene_tractor_seatbelt(d):
    """農道を走る乗用型トラクタと、ボンネットの義務車ステッカー、緑の標識を描いた場面。"""
    sky(d)
    mountains(d)
    ...
    d.polygon([(0, 334), (760, 324), (760, BAND_TOP), (0, BAND_TOP)], fill=(214, 227, 200))
    # 左：茶畑の畝
    d.polygon([(0, 344), (250, 336), (258, 404), (0, 412)], fill=(143, 184, 98))
    for i in range(3):
        d.line([(0, 356 + i * 20), (254, 348 + i * 20)], fill=(95, 138, 66), width=5)
    # 右：路肩の下の水田
    d.polygon([(540, 366), (760, 356), (760, 424), (552, 436)], fill=(169, 207, 125))
    for i in range(2):
        d.line([(546, 388 + i * 20), (760, 378 + i * 20)], fill=(109, 154, 76), width=4)
    # 農道
    d.polygon([(0, 414), (536, 392), (580, 430), (760, 486), (760, BAND_TOP), (0, BAND_TOP)],
              fill=(203, 191, 164))
    d.polygon([(536, 392), (580, 430), (556, 442), (520, 414)], fill=(179, 162, 132))
    # ロータリー作業機
    d.rectangle([104, 432, 210, 486], fill=(176, 58, 46), outline=(125, 40, 31), width=4)
    d.rectangle([104, 476, 210, 494], fill=(74, 80, 87))
    for x in (120, 148, 176, 200):
        d.line([(x, 494), (x - 5, 512)], fill=(59, 64, 70), width=6)
    # 緑の標識
    d.rectangle([106, 452, 148, 480], fill=(47, 125, 70), outline=(30, 91, 49), width=3)
    # 後輪
    d.ellipse([196, 430, 322, 556], fill=(51, 56, 62), outline=(30, 34, 38), width=6)
    d.ellipse([226, 460, 292, 526], fill=(201, 204, 199), outline=(142, 147, 141), width=5)
    d.ellipse([250, 484, 268, 502], fill=(125, 130, 124))
    # 車体
    d.polygon([(180, 384), (390, 376), (416, 414), (416, 470), (180, 478)], fill=(176, 58, 46))
    d.polygon([(180, 448), (416, 440), (416, 470), (180, 478)], fill=(143, 47, 38))
    # ボンネット
    d.polygon([(296, 384), (426, 378), (434, 436), (296, 440)], fill=(201, 66, 47))
    d.rectangle([426, 396, 442, 424], fill=(63, 69, 80))
    # 義務車ステッカー
    d.rectangle([320, 392, 412, 430], fill=(255, 255, 255), outline=(51, 58, 51), width=3)
    d.ellipse([330, 398, 344, 412], fill=(51, 58, 51))
    d.polygon([(326, 416), (348, 416), (348, 428), (326, 428)], fill=(51, 58, 51))
    d.text((356, 402), "義務車", font=ImageFont.truetype(FONT_BOLD, 17), fill=(51, 58, 51))
    # 安全フレームとキャブ
    d.line([(194, 384), (194, 258)], fill=(109, 115, 121), width=9)
    d.line([(296, 380), (296, 254)], fill=(109, 115, 121), width=9)
    d.line([(194, 258), (296, 254)], fill=(109, 115, 121), width=9)
    d.polygon([(202, 268), (288, 264), (288, 374), (202, 378)], fill=(206, 222, 231))
    # 運転者とシートベルト
    d.ellipse([226, 288, 268, 330], fill=(232, 201, 166), outline=(185, 151, 114), width=3)
    d.polygon([(224, 300), (270, 300), (266, 284), (228, 284)], fill=(63, 92, 104))
    d.polygon([(214, 336), (280, 336), (288, 378), (206, 378)], fill=(91, 127, 146))
    d.line([(220, 338), (270, 376)], fill=(44, 50, 56), width=11)
    d.rectangle([260, 366, 280, 382], fill=(192, 57, 43))
    # 前輪
    d.ellipse([392, 470, 470, 548], fill=(51, 56, 62), outline=(30, 34, 38), width=5)
    d.ellipse([414, 492, 448, 526], fill=(201, 204, 199), outline=(142, 147, 141), width=4)
    # 手前：銘板を書き写したノート
    d.rounded_rectangle([500, 456, 726, 578], radius=6, fill=(255, 255, 255),
                        outline=(138, 148, 154), width=4)
    d.rectangle([520, 472, 706, 492], fill=(223, 230, 233))
    for i in range(4):
        d.line([(520, 510 + i * 16), (632, 510 + i * 16)], fill=(198, 206, 210), width=5)
        d.line([(650, 510 + i * 16), (706, 510 + i * 16)], fill=(214, 168, 60), width=5)
...
def scene_otameshi_iju_nashi(d):
    ...
    # 県の一覧を模した掲示板
    d.rectangle([52, 336, 708, 502], fill=(246, 242, 230), outline=(150, 132, 104), width=5)
    d.rectangle([52, 336, 708, 368], fill=(31, 62, 82))
    d.text((66, 342), "静岡県　お試し住宅一覧　11市町13施設",
           font=ImageFont.truetype(FONT_BOLD, 20), fill="#ffffff")
    slots = [(78 + c * 88, 388 + r * 52) for r in range(2) for c in range(7)]
    ...
    d.text((406, 472), "森町は掲載なし",
           font=ImageFont.truetype(FONT_BOLD, 20), fill=(166, 59, 49))
    # 手前：計画に載る2棟
    for x, label in ((80, "鍛治島"), (180, "天宮")):
        d.rectangle([x, 534, x + 76, 578], fill=(233, 240, 228), outline=(76, 122, 96), width=4)
        d.polygon([(x - 8, 536), (x + 38, 510), (x + 84, 536)], fill=(126, 168, 92),
                  outline=(76, 122, 96))
        d.text((x + 10, 548), label, font=ImageFont.truetype(FONT_BOLD, 18), fill=(51, 80, 63))
    d.text((272, 518), "「整備済」の2棟が計画にある",
           font=ImageFont.truetype(FONT_BOLD, 23), fill=(31, 62, 82))
    d.text((272, 552), "料金も申込先も公表なし",
           font=ImageFont.truetype(FONT_BOLD, 23), fill=(166, 59, 49))
    band(d, ["計画には2棟ある。", "案内はどこにもない"],
         "森町ライフハック／移住・暮らし・データ")
