...
S1 = "https://www.town.morimachi.shizuoka.jp/gyosei/machinososhiki/teijusuishinka/ijukoryugakari/2/2/3043akiyakazaisyobun.html"
S2 = "https://www.town.morimachi.shizuoka.jp/material/files/group/10/R3kouhuyoukou.pdf"
...
S5 = "https://www.mlit.go.jp/jutakukentiku/house/jutakukentiku_house_tk4_000001.html"
S6 = "https://www.mlit.go.jp/common/000040544.pdf"
...
for (_, paras), extra in zip(sections, supplements):
    paras.append(extra)
...
for (_, paras), extra in zip(sections, detail_notes):
    paras.append(extra)

facts = [
    ("森町の支援案内は交付決定後の着工から実績報告・完了検査・額確定・支払いまでの流れを示す", S1),
    ("森町の支援対象には台所・風呂・トイレ・電気・ガス・水道・内装・屋根・外壁の改修が含まれる", S1),
    ("森町の支援対象には残置物処分・清掃・支障木伐採・除草等も含まれる", S1),
    ("森町の交付要綱は実績報告へ領収書の写しを添付すると定める", S2),
    ("森町の交付要綱は実績報告へ着手前と完了後の写真を添付すると定める", S2),
    ("森町の交付要綱には実績報告書・請求書等の様式がある", S2),
    ("森町の空き家確認票は雨漏り・床・白蟻・傾き腐食を別項目にする", S3),
    ("森町の空き家確認票は外壁・躯体基礎・害獣・残置物を別項目にする", S3),
    ("森町の空き家確認票は水道種類と下水道・浄化槽・くみ取り等を分ける", S3),
    ("森町の第2期空家等計画は管理状況と利活用可能性に応じて助言・支援等を分ける", S4),
    ("森町は令和4年度に町内全域の空き家等実態調査を実施した", S4),
    ("国土交通省は点検・修繕・リフォーム等の記録を住宅履歴情報と説明する", S5),
    ("国土交通省は住宅履歴情報を点検・不具合確認・リフォーム・売却に活用できるとする", S5),
    ("国の住宅履歴情報項目例には完了日・工事業者・工事内容・見積書・領収書が含まれる", S6),
    ("国の住宅履歴情報項目例には施工図・仕様書・工事記録写真・打合せ記録が含まれる", S6),
]
...
record = {
    ...
    "morimachi_conditions": [
        "町への実績報告資料と家族保存用の履歴を分ける",
        "着手前・施工中・完了後を同じ部位番号で結ぶ",
        "施工済み・調査のみ・未実施・経過観察を分ける",
        "修繕履歴と道路・境界・登記等の確認を混同しない",
    ],
    "section_headings": [h for h, _ in sections],
    "section_paragraphs": [{"heading": h, "paragraphs": p} for h, p in sections],
    "faqs": [
        {"question": "空き家の修繕履歴には何を一行で残しますか？", "answer": "施工日、部位、症状、実施内容、施工者、費用、着手前・施工中・完了後の写真番号、保証、未実施項目、次回確認日を一件番号で結びます。"},
        {"question": "森町の補助へ出した写真だけ保存すれば足りますか？", "answer": "町への提出資料と家族用履歴は目的が異なります。提出要件を町へ確認したうえで、家族用には施工中写真、保証書、対象外項目、打合せ、次回点検も残してください。"},
        {"question": "見積書と領収書は同じ役割ですか？", "answer": "異なります。見積書は予定範囲、契約書等は合意範囲、請求書は請求内容、領収書や振込記録は支払の証拠として別々に照合します。"},
        {"question": "修繕履歴があれば建物全体を確認済みと説明できますか？", "answer": "できません。履歴は記録された部位と時点の資料です。未実施、未確認、調査の限界を残し、現在の状態や土地・権利関係は必要に応じて改めて確認してください。"},
    ],
    "sources": [
        {"url": S1, "title": "森町 空き家等利活用推進支援費用補助金", "status": "verified-http-200", "checked_at": "2026-08-16"},
        {"url": S2, "title": "森町 空き家等利活用推進支援事業費補助金交付要綱", "status": "verified-http-200", "checked_at": "2026-08-16"},
        {"url": S3, "title": "森町 空き家・空き地バンク確認票", "status": "verified-http-200", "checked_at": "2026-08-16"},
        {"url": S4, "title": "森町 第2期空家等計画", "status": "verified-http-200", "checked_at": "2026-08-16"},
        {"url": S5, "title": "国土交通省 住宅履歴情報とは", "status": "verified-http-200", "checked_at": "2026-08-16"},
        {"url": S6, "title": "国土交通省 住宅履歴情報の項目例", "status": "verified-http-200", "checked_at": "2026-08-16"},
    ],
}

(ROOT / "work" / "day57-research-125.json").write_text(
...
for topic in topics:
    if topic["id"] == 125:
        topic["title"] = "森町の空き家修繕後に残す｜写真・領収書・保証の履歴台帳"
        topic["search_intent"] = "空き家の修繕完了後に、施工部位・着手前後写真・領収書・保証・未実施箇所を一件台帳へ残したい"
        topic["primary_sources"] = [S1, S2, S3, S4, S5, S6]
(ROOT / "data" / "seo-phase4-topics.json").write_text(
    json.dumps(topics, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
)
print("day57 proposal written")

def update_public_surfaces():
    page_url = "/vacant-house/vacant-house-maintenance-log/"
    ...
    if full_url not in sitemap:
        sitemap = sitemap.replace(
            "</urlset>",
            f'<url><loc>{full_url}</loc><lastmod>2026-08-16</lastmod></url>\n</urlset>',
        )
        sitemap_path.write_text(sitemap, encoding="utf-8")

    search_path = ROOT / "search-index.json"
    search = json.loads(search_path.read_text(encoding="utf-8"))
    if not any(item.get("href") == page_url for item in search):
        search.append({
            "href": page_url,
            "icon": "🏠",
            "title": "森町の空き家修繕後に残す｜写真・領収書・保証の履歴台帳",
            "category": "空き家",
            "hub": "property",
            "kind": "記録",
            "summary": "施工部位、着手前後写真、領収書、保証、未実施箇所を一件台帳にする",
            "keyword": "森町 空き家 修繕履歴 写真 領収書 保証",
            "aliases": ["空き家修繕履歴", "工事写真台帳", "住宅履歴情報"],
            "needs": ["空き家の修繕記録を家族へ残したい"],
            "department": ["定住推進課"],
            "audience": ["空き家所有者", "遠方の家族"],
            "verified": "2026-08-16",
            "lead": "空き家の修繕完了後に、施工部位、着手前・施工中・完了後の写真、領収書、保証、未実施箇所、次回確認日を一件番号で結ぶ実務台帳です。",
        })
        search_path.write_text(json.dumps(search, ensure_ascii=False, separators=(",", ":")), encoding="utf-8")

    index_path = ROOT / "index.html"
    index = index_path.read_text(encoding="utf-8")
    index = index.replace("サイト全体の公開ページは 781 件です", "サイト全体の公開ページは 782 件です")
